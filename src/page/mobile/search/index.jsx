    ...
    render() {
        ...
        const content = this.state.data.rows && this.state.data.rows.map((item, index) => {
            ...
            return <Link to={{pathname: `/product/${item.specialty.id}`}} key={index}>
                <Flex style={{background:'#fff'}}>
                    <Flex.Item style={{flex: '0 0 30%'}}>
                        <img src={"http://" + getServerIp() + item.iconURL.thumbnailPath} style={{width: '70%', margin:'0.4rem'}}/>
                    </Flex.Item>
                    <Flex.Item style={{flex: '0 0 60%', color:'black'}}>
                        ...
                        <div>销量：<span style={{color:'darkorange'}}>{item.specification.hasSold}</span></div>
                        <WhiteSpace/>
                    </Flex.Item>
                </Flex>
                <WhiteSpace/>
